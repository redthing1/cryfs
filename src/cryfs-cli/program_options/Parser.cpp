...
ProgramOptions Parser::parse(const vector<string> &supportedCiphers) const {
    ...
    if (vm.count("foreground")) {
        LOG(WARN, "The `-f` flag currently does not have an effect on Windows. Cryfs will always run in foreground. Future versions may change that behavior.");
    }
    ...
    return ProgramOptions(std::move(baseDir), std::move(mountDir), std::move(configfile), foreground, allowReplacedFilesystem, createMissingBasedir, createMissingMountpoint, std::move(unmountAfterIdleMinutes), std::move(logfile), std::move(cipher), blocksizeBytes, std::move(fuseOptions));
}
