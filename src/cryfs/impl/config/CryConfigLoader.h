...
public:
  ...
  CryConfigLoader(std::shared_ptr<cpputils::Console> console, cpputils::RandomGenerator *keyGenerator, cpputils::unique_ref<CryKeyProvider> keyProvider, LocalStateDir localStateDir, const boost::optional<std::string> &cipherFromCommandLine, const boost::optional<uint32_t> &blocksizeBytesFromCommandLine);
  ...
  cpputils::either<CryConfigFile::LoadError, CryConfigLoader::ConfigLoadResult> loadOrCreate(boost::filesystem::path filename, bool allowReplacedFilesystem);
  cpputils::either<CryConfigFile::LoadError, CryConfigLoader::ConfigLoadResult> load(boost::filesystem::path filename, bool allowReplacedFilesystem, CryConfigFile::Access access);

private:
    cpputils::either<CryConfigFile::LoadError, CryConfigLoader::ConfigLoadResult> _loadConfig(boost::filesystem::path filename, bool allowReplacedFilesystem, CryConfigFile::Access access);
    ConfigLoadResult _createConfig(boost::filesystem::path filename, bool allowReplacedFilesystem);
    void _checkVersion(const CryConfig &config) const;
