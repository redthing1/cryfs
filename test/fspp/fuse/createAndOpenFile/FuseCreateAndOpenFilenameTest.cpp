...
TEST_F(FuseCreateAndOpenFilenameTest, CreateAndOpenFile) {
  ...
  EXPECT_CALL(*fsimpl, createAndOpenFile(Eq("/myfile"), testing::_, testing::_, testing::_, testing::_))
...
TEST_F(FuseCreateAndOpenFilenameTest, CreateAndOpenFileNested) {
  ...
  EXPECT_CALL(*fsimpl, createAndOpenFile(Eq("/mydir/myfile"), testing::_, testing::_, testing::_, testing::_))
    .Times(1).WillOnce(Return(0));
  //For the syscall to succeed, we also need to give an fstat implementation.
  ReturnIsFileOnFstat(0);
...
TEST_F(FuseCreateAndOpenFilenameTest, CreateAndOpenFileNested2) {
  ...
  EXPECT_CALL(*fsimpl, createAndOpenFile(Eq("/mydir/mydir2/myfile"), testing::_, testing::_, testing::_, testing::_))
    .Times(1).WillOnce(Return(0));
  //For the syscall to succeed, we also need to give an fstat implementation.
  ReturnIsFileOnFstat(0);
